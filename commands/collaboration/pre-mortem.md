---
name: pre-mortem
description: "Imagine failure first to surface mitigations that double as feature ideas"
category: collaboration
complexity: standard
mcp-servers: []
personas: [architect, security, devops, product-manager]
subagents: []
---

# /collaboration:pre-mortem — Skill-backed pre-mortem

## Use
- Load `skills/collaboration/pre_mortem/SKILL.md` (skill `pre-mortem`) and follow it.
- Keep the failure modes → mitigations structure from the skill.

## Usage
```
/collaboration:pre-mortem [initiative] [--horizon 30|90|180]
```
