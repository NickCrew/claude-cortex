---
name: metrics
description: "Track reasoning effectiveness and optimization metrics across commands"
category: utility
complexity: basic
mcp-servers: []
personas: [data-analyst, performance-engineer, cost-optimizer]
subagents: []
---

# /reasoning:metrics — Skill-backed reasoning control

## Use
- Load `skills/reasoning-controls/SKILL.md` (skill `reasoning-controls`) and follow it.
- Then load `skills/reasoning-controls/references/metrics.md` for the detailed workflow.

## Usage
```
/reasoning:metrics [--show] [--reset]
```
